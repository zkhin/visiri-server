...
const ExperimentsService = {
  getAllexperiments(db) {
    return db
      ...
      .leftJoin("experiment_regions AS reg", "exp.id", "reg.experiment_id")
      .leftJoin("visiri_users AS usr", "exp.user_id", "usr.id")
  ...
  serializeExperiments(experiments) {
    return experiments.map(experiment =>
      ExperimentsService.serializeExperiment(experiment)
    );
  },
